...
void devicecheck::on_pushButton_insert_device_clicked()
{
    ...
    query.prepare("call device_management.sp_insert_device(:_modle, :_pn, :_sn)");
...
void devicecheck::on_pushButton_query_device_clicked()
{
    ...
    model->setQuery("select * from device_management.view_device_human_sa where modle like '%"+modle+"%'");
...
void devicecheck::on_pushButton_update_device_clicked()
{
    QSqlQuery query;
    QString sn = ui->lineEdit_SN->text();
    query.prepare("call device_management.sp_update_device(:_sn)");
